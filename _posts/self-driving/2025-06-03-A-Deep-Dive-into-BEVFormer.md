...
In `BEVFormer-base`, the backbone network is `ResNet-101` with `DCN` (Deformable Convolutional Network). This means we use `DCN` layers instead of standard `Conv2D` layers in key parts of the network to better capture geometric variations.
...
`DCN` introduces two key enhancements:
...
By using `DCN` in the backbone, `BEVFormer` can extract spatially adaptive features that better preserve object shapes and locations, improving the performance of downstream tasks such as BEV transformation and 3D object detection.

The c++ implementation of `DCN` in `MMCV` can be found [here](https://github.com/open-mmlab/mmcv/blob/v1.4.0/mmcv/ops/csrc/parrots/modulated_deform_conv_cpu.cpp#L144-L166).
...
1. `Backbone`:
Extracts multi-scale image features from input camera images using a deep convolutional neural network, such as `ResNet-101` with deformable convolutions (`DCN`).
2. `Neck`:
...
To ensure spatial consistency, the previous BEV features must be aligned with the current coordinate frame. This alignment is performed using the `can_bus` vector, which encodes the ego vehicle’s motion (e.g., translation and rotation). You can find the relevant implementation here.
...
<div style="text-align: center;">
  <figure style="display: inline-block; margin: 0;">
    <img src="https://github.com/jysh1214/jysh1214.github.io/blob/master/_assets/2025-06-03-A-Deep-Dive-into-BEVFormer/bev_plane.png?raw=true" width="250"/>
    <figcaption style="font-size: 14px; color: #555;">(image by author)</figcaption>
  </figure>
</div>
...
The following image shows a visualization (by cv2) of the corresponding queries for cameras 1–6:
